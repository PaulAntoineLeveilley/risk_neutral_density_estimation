...
def rnd_heston(S: float, V: float, T: float, r: float, kappa: float, theta: float, lambd: float, rho: float, sigma: float, upper_bound: float, x: float):
    """
    Computes the risk neutral density of the Heston model
    ...
    """
    def integrand(phi): return (np.exp(-1j*phi*np.log(x)) *
                                f(2, S, V, T, r, kappa, theta, lambd, rho, sigma, phi)).real
...
def P(i: int, S: float, T: float, V: float, K: float, r: float, kappa: float, theta: float, lambd: float, rho: float, sigma: float, upper_bound: float):
    ...
    """
    def integrand(phi): return (np.exp(-1j*phi*np.log(K))*f(i, S,
                                                            V, T, r, kappa, theta, lambd, rho, sigma, phi)/(1j*phi)).real
    integral = quad(integrand, 0, upper_bound)[0]
