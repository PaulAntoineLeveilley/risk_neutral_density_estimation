...
def rnd_bakshi(S: float, V: float, T: float, r: float, kappa: float, theta: float, lambd: float, rho: float, sigma: float, muj: float, sigmaj: float, upper_bound: float, x: float):
    """
    ...
    """
    def integrand(phi): return (np.exp(-1j*phi*np.log(x))*f(2, S, V,
                                                            T, r, kappa, theta, lambd, rho, sigma, muj, sigmaj, phi)).real
...
def f(i: int, S: float, V: float, T: float, r: float, kappa: float, theta: float, lambd: float, rho: float, sigma: float, muj: float, sigmaj: float, phi: complex):
    ...
    """
    C, D, E = compute_C_D_E(i, T, r, kappa, theta, lambd,
                            rho, sigma, muj, sigmaj, phi)
